...
import { DEFAULT_LABEL_TEMPLATE, LABEL_FIELD_OPTIONS, type LabelField, type LabelPriceMode, type LabelTemplate } from "@/lib/labels/types";
    ...
    {loading && !report ? <div className="py-16 text-center text-gray-500">Loading barcodes...</div> : <>
      ...
      {tab === "labels" && <div className="space-y-4">
        <section className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold text-gray-950">{lang === "sw" ? "Chapisha labels" : "Print labels"}</h2><p className="text-xs text-gray-500">{lang === "sw" ? "Chagua bidhaa, muonekano, ukubwa na idadi." : "Choose products, fields, size, and copies."}</p></div><button disabled={!printProducts.length} onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-40"><Printer className="h-4 w-4" />Print ({printProducts.length})</button></div>
        ...
        <div className="print-labels hidden print:grid print:grid-cols-2 print:gap-2">{printProducts.map((product, index) => <BarcodeLabel key={`${product.id}-${index}`} value={product.barcode} name={product.name} price={labelPrice(product)} sku={product.sku} unit={product.unit} customText={template.customText} fields={template.fields} widthMm={template.widthMm} heightMm={template.heightMm} className="label-print-item border border-gray-300" />)}</div>
      </div>}
